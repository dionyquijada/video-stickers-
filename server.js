...
Diony Quijada, [2/7/2025 8:39 p. m.]
const express = require('express'); const ffmpeg = require('fluent-ffmpeg'); const ytdl = require('ytdl-core'); const path = require('path'); const fs = require('fs'); const app = express(); app.use(express.json()); // Servir archivos estáticos (HTML, CSS, JS) app.use(express.static('.')); app.post('/api/generate-sticker', async (req, res) => { const { url, startTime, endTime } = req.body; const outputPath = path.join(__dirname, output-${Date.now()}.mp4); try { let videoStream; if (url.includes('youtube.com')  url.includes('youtu.be')) { videoStream = ytdl(url, { filter: 'videoandaudio' }); } else { videoStream = url; } ffmpeg(videoStream) .setStartTime(startTime) .setDuration(endTime - startTime) .outputOptions('-c:v libx264') .outputOptions('-c:a aac') .save(outputPath) .on('end', () => { res.sendFile(outputPath, (err) => { if (err) { res.status(500).send('Error al enviar el archivo'); } fs.unlinkSync(outputPath); }); }) .on('error', (err) => { console.error(err); res.status(500).send('Error al procesar el video'); }); } catch (error) { console.error(error); res.status(500).send('Error en el servidor'); } }); app.listen(process.env.PORT  3000, () => console.log('Servidor corriendo'));
